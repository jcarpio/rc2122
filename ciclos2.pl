...
camino(Ini, Fin, g(_,A), Visitados, [Ini|Camino]):-
  ...
  camino(TMP, Fin, g(_, A), [TMP|Visitados], Camino).
...
grafo1(g(_,A)), camino(a, a, g(_,A), [a], Camino), length(Camino, Length), Length > 3.

 grafo1(g(V,A)), member(N, V), setof(Camino, camino(N, N, g(_,A), [], Camino), R).
